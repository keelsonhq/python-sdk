...
def test_keelson_mode_missing_identity_fails_closed(monkeypatch) -> None:
    ...
    with pytest.raises(FilesError, match="platform identity is missing"):
        _resolve_mode()
...
@pytest.mark.parametrize(
    "var", ["KEELSON_APP_ID", "KEELSON_TENANT_ID", "KEELSON_DEPLOY_ID"]
)
def test_unset_mode_refuses_fallback_on_platform(monkeypatch, var: str) -> None:
    _set(monkeypatch, **{var: "x"})
    with pytest.raises(FilesError, match="refusing to fall back"):
        _resolve_mode()
